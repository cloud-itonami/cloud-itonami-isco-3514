(ns webtech.actor
  "WebTechniciansActor — the ISCO-08 3514 community webtech actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one webtech operation request
  ...
  The unconditional invariant: the WebTechniciansAdvisor can never
  directly commit a record the WebTechniciansGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [webtech.advisor :as advisor]
            [webtech.governor :as governor]
            [webtech.store :as store]))

(defn build-graph
  "Build a compiled WebTechniciansActor graph. `store` implements
  `webtech.store/Store`. `advisor` implements
  `webtech.advisor/Advisor` (defaults to `mock-advisor`).
